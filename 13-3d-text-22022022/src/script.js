...
fontLoader.load('/fonts/Digital-7_Regular.json', (font) => {
    ...
    //     bevelSegments: 5,
    // })
    const textGeometries = parameters.text.split('').map(t => {
        return new TextGeometry(t, {
            font,
            size: 0.5,
            height: 0.2,
            curveSegments: 6,
    ...
    // )
    textGeometries.forEach(tGeom => {
        tGeom.center();
        texts.push(
            new THREE.Mesh(tGeom, textMaterial)
        );
    });
    group.add(...texts);
    texts.forEach((text, idx) => {
        text.position.x = (idx * 0.5) - 1.5
    })
